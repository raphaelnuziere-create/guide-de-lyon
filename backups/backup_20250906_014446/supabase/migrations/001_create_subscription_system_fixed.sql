...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'event_published_trigger') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'establishment_slug_trigger') THEN
...
-- 13. CRÉATION DU BUCKET STORAGE SI NÉCESSAIRE
-- =====================================================
INSERT INTO storage.buckets (id, name, public)
VALUES ('establishment-media', 'establishment-media', true)
ON CONFLICT (id) DO NOTHING;

-- Politique pour le bucket
CREATE POLICY "Public can view establishment media" ON storage.objects
  FOR SELECT USING (bucket_id = 'establishment-media');

CREATE POLICY "Users can upload their establishment media" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'establishment-media' 
    AND auth.uid()::text = (storage.foldername(name))[1]
  );

CREATE POLICY "Users can update their establishment media" ON storage.objects
  FOR UPDATE USING (
    bucket_id = 'establishment-media' 
    AND auth.uid()::text = (storage.foldername(name))[1]
  );

CREATE POLICY "Users can delete their establishment media" ON storage.objects
  FOR DELETE USING (
    bucket_id = 'establishment-media' 
    AND auth.uid()::text = (storage.foldername(name))[1]
  );