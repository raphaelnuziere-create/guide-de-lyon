-- Nettoyer et réinsérer les articles de test
-- À exécuter pour avoir des articles visibles immédiatement

-- Désactiver RLS temporairement
ALTER TABLE scraped_articles DISABLE ROW LEVEL SECURITY;

-- Supprimer les anciens articles de test s'ils existent
DELETE FROM scraped_articles WHERE original_url IN (
  'https://test1.com',
  'https://test2.com', 
  'https://test3.com'
);

-- Supprimer les articles avec des valeurs nulles
DELETE FROM scraped_articles WHERE rewritten_title IS NULL;

-- Insérer les nouveaux articles de test
...
-- Compter le total
SELECT COUNT(*) as "Total articles publiés" 
FROM scraped_articles 
WHERE status = 'published';

-- Message de confirmation
SELECT '✅ Articles de test insérés avec succès! Consultez https://www.guide-de-lyon.fr/actualites' as message;