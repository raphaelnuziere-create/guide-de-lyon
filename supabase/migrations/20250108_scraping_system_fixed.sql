-- Migration: Système complet de scraping d'actualités (CORRIGÉ)
-- Date: 2025-01-08
...
CREATE TABLE IF NOT EXISTS scraped_articles (
  ...
  -- Auteur
  author_name VARCHAR(100) DEFAULT 'Raphael',
  author_bio TEXT DEFAULT 'Rédacteur en chef du Guide de Lyon',
...
DROP TRIGGER IF EXISTS update_scraped_articles_updated_at ON scraped_articles;
...
INSERT INTO scraping_sources (name, url, type, selectors, frequency_minutes) 
VALUES
(
  'Le Progrès Lyon RSS',
...
-- Policy pour admin (basée sur auth.uid() seulement)
CREATE POLICY "Admin peut tout faire sur scraped_articles" ON scraped_articles
  FOR ALL
  USING (
    auth.uid() IS NOT NULL 
    AND auth.uid() IN (
      SELECT user_id FROM establishments WHERE plan IN ('expert', 'admin')
    )
  );

CREATE POLICY "Admin peut tout faire sur scraping_sources" ON scraping_sources
  FOR ALL
  USING (
    auth.uid() IS NOT NULL 
    AND auth.uid() IN (
...
CREATE POLICY "Admin peut tout faire sur scraping_queue" ON scraping_queue
  FOR ALL
  USING (
    auth.uid() IS NOT NULL 
    AND auth.uid() IN (
...
CREATE POLICY "Admin peut tout faire sur scraping_logs" ON scraping_logs
  FOR ALL
  USING (
    auth.uid() IS NOT NULL 
    AND auth.uid() IN (
...
COMMENT ON COLUMN scraped_articles.author_name IS 'Auteur de l''article (toujours Raphael)';